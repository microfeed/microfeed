...
export const CLI_HELP_TOPICS: readonly CliHelpTopic[] = [
  ...
  },
  {
    details: [
      ...
      "Plain listening remains loopback-only. Add --tunnel to launch a free, temporary Cloudflare Quick Tunnel, print its random HTTPS /webhook URL, and receive a deployed instance's signed test on this computer. The tunnel is public only while the command runs and is for development, not production.",
      "Tunnel mode uses cloudflared from PATH or a versioned microfeed cache. When it is missing, the interactive CLI offers to download a pinned official binary after confirmation and verifies its SHA-256 digest before caching it. Use --install-cloudflared for explicit non-interactive approval or --cloudflared-path for a managed executable.",
      "The listener never creates or edits a microfeed endpoint. Register the printed URL in Admin → Webhooks → Endpoints, reveal its signing secret, and paste that secret into the visible prompt. Stopping the listener also stops its temporary tunnel.",
    ...
    options: [
      ...
      option("--tunnel", "Expose the listener through a temporary Cloudflare Quick Tunnel."),
      option("--install-cloudflared", "With --tunnel, approve downloading the pinned verified helper without a prompt."),
      option("--cloudflared-path <path>", "With --tunnel, use this managed cloudflared executable."),
      option("--json", "Write one NDJSON object per verified delivery to stdout."),
    ],
    path: ["webhook"],
    subcommands: [
      {name: "listen", description: "Verify, display, and optionally forward signed webhook deliveries."},
  ...
  },
  {
    ...
    path: ["webhook", "listen"],
    summary: "Verify and display webhook deliveries locally or through a temporary tunnel.",
