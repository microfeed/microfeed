...
import {NAV_ITEMS, NAV_ITEMS_DICT, OUR_BRAND} from "@/shared/Constants";
import {authClient} from "@/client/auth-client";

function NavItem({url, title, navId, currentId, Icon, disabled}: any) {
  return (
    <a href={disabled ? '#' : url}>
      <div
...
export default class AdminNavApp extends React.Component<any, any> {
  ...
  async logout() {
    ...
    location.href = ADMIN_URLS.login();
  }
      ...
      <div className="grid grid-cols-12 gap-4 bg-white flex items-center border-b drop-shadow-sm">
        ...
        <div className="col-span-10 flex items-center">
          ...
          <div className="flex-1 text-right py-6 px-4 xl:px-16">
            <button
              className="hover:opacity-50 text-brand-dark font-semibold text-sm"
              onClick={() => void this.logout()}
              type="button"
            >
              <div className="flex items-center justify-end">
                <div className="mr-1"><ArrowLeftStartOnRectangleIcon className="w-4"/></div>
                <div>Logout</div>
              </div>
            </button>
          </div>
      ...
      <div className="grid grid-cols-12 gap-4 flex-1">
        <div className="col-span-2 bg-brand-dark flex-none">
          <nav className="py-8">
            <NavItem
              url={ADMIN_URLS.home()}
              title={(NAV_ITEMS_DICT[NAV_ITEMS.ADMIN_HOME] as any).name}
            ...
            <NavItem
              ...
              title={(NAV_ITEMS_DICT[NAV_ITEMS.EDIT_CHANNEL] as any).name}
            ...
            <NavItem
              ...
              title={(NAV_ITEMS_DICT[NAV_ITEMS.NEW_ITEM] as any).name}
            ...
            <NavItem
              ...
              title={(NAV_ITEMS_DICT[NAV_ITEMS.ALL_ITEMS] as any).name}
            ...
            <NavItem
              ...
              title={(NAV_ITEMS_DICT[NAV_ITEMS.SETTINGS] as any).name}
