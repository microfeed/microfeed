...
describe("source architecture", () => {
  it("has no legacy source roots or references", async () => {
    ...
    const contents = [
      ...
      await contentsUnder(path.join(repositoryRoot, "admin-cli")),
    ...
    for (const directory of legacyRoots.slice(0, 3)) {
  ...
  it("keeps browser, Worker, and repository tooling boundaries separate", async () => {
    ...
    const adminCliContents = await contentsUnder(
      path.join(repositoryRoot, "admin-cli"),
    );
    ...
    expect(adminCliContents).not.toContain('from "@/server/');
    expect(adminCliContents).not.toContain('from "@/client/');
    expect(adminCliContents).not.toContain('from "@/components/');
  });
