...
import type {CommandRunner} from "../../../admin-cli/types";
...
async function freshModules() {
  ...
  // `yarn admin setup --instance <name>` runs the suite with this variable
  // set to its deployment target. These tests exercise instance selection
  // themselves, so they must not inherit the outer administration command.
  ...
  const commands = await import("../../../admin-cli/commands");
  const config = await import("../../../admin-cli/lib/config");
  const promptModule = await import("../../../admin-cli/lib/prompts");
...
describe("first-class local instances", () => {
  it("creates, resumes, and isolates multiple named local instances", async () => {
    ...
    const personalInstanceId = personal!.instanceId;
    await commands.setupCommand(
      {instance: "personal", local: true},
      runner,
    );
    await expect(config.readConfig(false, "personal")).resolves.toEqual(
      expect.objectContaining({instanceId: personalInstanceId}),
    );
  ...
  it("can skip local login and enable it later for the same instance", async () => {
    ...
    expect(setupOutput).toContain(
      "yarn admin auth setup --local --instance",
    );
