...
} from "../../../admin-cli/lib/cloudflare";
import type {
  CommandRunner,
  MicrofeedConfig,
} from "../../../admin-cli/types";
import {repositoryRoot} from "../../../admin-cli/lib/process";
...
describe("CloudflareClient", () => {
  ...
  it("lists secret names and deletes setup secrets without their values", async () => {
    const runner = vi.fn<CommandRunner>()
      .mockResolvedValueOnce(commandResult(JSON.stringify([
        {name: "BETTER_AUTH_SECRET", type: "secret_text"},
        {name: "MICROFEED_SETUP_ADMIN_EMAIL", type: "secret_text"},
      ])))
      .mockResolvedValueOnce(commandResult("Secrets updated"));
    const cloudflare = new CloudflareClient(runner);

    await expect(cloudflare.workerSecretNamesFromBuild()).resolves.toEqual(
      new Set([
        "BETTER_AUTH_SECRET",
        "MICROFEED_SETUP_ADMIN_EMAIL",
      ]),
    );
    await cloudflare.deleteWorkerSecretsFromBuild([
      "MICROFEED_SETUP_ADMIN_EMAIL",
    ]);
    ...
    expect(runner).toHaveBeenNthCalledWith(
      ...
      expect.objectContaining({
        input: '{"MICROFEED_SETUP_ADMIN_EMAIL":null}',
      }),
    );
  });
