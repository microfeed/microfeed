...
export const CLI_COMMANDS: readonly CliCommandMetadata[] = [
  ...
  },
  {
    ...
    details: [
      ...
      "Before changing webhook infrastructure, deploy verifies the existing exact Queue when present and performs read-only Queue and D1 access checks. Each safe check retries once after Cloudflare authentication code 10000. A repeated rejection stops before mutation; a later rejection preserves a resumable transition and prints the exact command to rerun in a fresh process.",
  ...
  },
  {
    ...
    details: [
      ...
      "Install accepts the canonical Built-in sources bundled:default, bundled:podcast, bundled:blog, bundled:photo, bundled:video, bundled:curation, and bundled:changelog; the compatibility alias default; a local directory; or a public GitHub repository, directory, or microfeed-theme.json URL. Built-in sources load registered rendered packages from this checkout; GitHub refs use an anonymous temporary Git fetch to resolve an exact commit and validate the tree without depending on GitHub's unauthenticated REST API quota. Git must be available.",
      "Every install is inactive. Activate separately after previewing in Settings → Themes.",
      "An environment can keep 100 non-deleted Custom versions and 20 drafts. Built-in versions do not use the Custom quota.",
      ...
      "Delete rejects every Built-in version and the active Custom version, and requires --confirm to exactly match a Custom theme ID.",
    ],
    examples: [
