...
export class CloudflareClient {
  ...
  private async credentials(): Promise<CloudflareCredentials> {
    this.credentialsPromise ??= (async () => {
      ...
      if (result.exitCode !== 0) {
        throw new Error(
          "Wrangler is not signed in. Run `yarn admin connect` to sign in.",
        );
      }
      const data = parseJsonOutput<Record<string, unknown>>(result.stdout);
      if (
  ...
  async applyMigrationsByBinding(binding: string): Promise<void> {
    await runWrangler(
      this.runner,
      [
        "d1",
  ...
  async authOwner(
    ...
    if (result.exitCode !== 0) {
      ...
      throw new Error(result.stderr.trim() || "Unable to inspect admin login.");
    }
  ...
  async authOwnerByBinding(binding: string): Promise<AuthOwner | null> {
    const result = await runWrangler(
      this.runner,
      [
        "d1",
  ...
  async workerSecretNamesFromBuild(): Promise<Set<string>> {
    const result = await runWrangler(
      this.runner,
      [
        "secret",
  ...
  async deleteWorkerSecretsFromBuild(
    ...
    await runWrangler(
      this.runner,
      ["secret", "bulk", "--config", builtWranglerConfigPath],
      {
...
export function pagesCollisionMessage(name: string): string {
  return `A Cloudflare Pages project named \`${name}\` already exists. ` +
    "This setup will not modify it. Change CLOUDFLARE_PROJECT_NAME to a " +
...
export function pagesDomainAttachedMessage(
  ...
  return `Custom domain \`${hostname}\` is still attached to Cloudflare ` +
    ...
    "2. Come back and run `yarn admin domain` again.\n\n" +
