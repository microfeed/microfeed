...
function validateConfig(
  ...
  if (!isRecord(value) || !isRecord(value.d1) || !isRecord(value.r2)) {
    throw new Error(
      `Invalid ${source}. Run \`yarn admin setup\` to repair it.`,
    );
  }
...
async function migrateLegacyConfig(): Promise<void> {
  ...
  if (!isRecord(legacyValue)) {
    throw new Error(
      "Invalid .microfeed/config.json. Run `yarn admin setup` to repair it.",
    );
  }
  ...
  if (config && !allowLocal && isLocalOnly(config)) {
    throw new Error(
      `Instance \`${config.instanceName}\` is local only and has no ` +
        ...
        "Cloudflare instance with `yarn admin use <name>`.",
    );
  }
