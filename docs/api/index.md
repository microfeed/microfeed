...
- The optional authenticated **API** under `/api/` lets integrations read,
...
| Operation | Purpose |
| --- | --- |
| `GET /api/feed/` | Read the complete feed with cursor pagination. |
| `POST /api/items/` | Create an item. |
| `GET /api/items/{itemId}/` | Read one item. |
| `PUT /api/items/{itemId}/` | Update provided fields without clearing omitted fields. |
| `DELETE /api/items/{itemId}/` | Delete one item. |
| `PUT /api/channels/primary/` | Update the primary channel. |
| `POST /api/media_files/presigned_urls/` | Prepare a short-lived, same-origin media upload. |
...
`<site-origin>` with that instance’s public origin:

| Resource | URL |
| --- | --- |
| Interactive API docs | `<site-origin>/api/` |
| OpenAPI JSON | `<site-origin>/api/openapi.json` |
| OpenAPI YAML | `<site-origin>/api/openapi.yaml` |
| Compact agent reference | `<site-origin>/api/llms.txt` |
| Self-contained agent reference | `<site-origin>/api/llms-full.txt` |
