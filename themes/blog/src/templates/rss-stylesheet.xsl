  ...
  <xsl:template match="/">
    <html class="rss-document" lang="en">
      <head>
        ...
        <title><xsl:value-of select="rss/channel/title"/> · RSS edition</title>
        <style>/* microfeed:compiled-theme */</style>
      </head>
      <body class="rss-page">
        <main class="rss-container">
          <header class="rss-header">
            <p class="blog-rubric">RSS edition</p>
            ...
            <xsl:if test="rss/channel/link[1]"><p><a><xsl:attribute name="href"><xsl:value-of select="rss/channel/link[1]"/></xsl:attribute>Read on the web →</a></p></xsl:if>
          </header>
