...
- Perform every Cloudflare deployment change through `yarn admin`. Do not
  improvise with raw Wrangler commands, direct Cloudflare API calls, or a
  separate deployment implementation.
- Treat `docs/admin-cli.md` as the canonical command, option, side-effect, and
  ...
  and `admin-cli/help.ts` synchronized whenever CLI behavior changes.
- Never delete or overwrite an unrelated Cloudflare resource. Let the admin
  CLI enforce its collision, reuse, and resume checks.
- For removal, first run `yarn admin destroy --dry-run` and relay its complete
  ...
  data-deletion scope before running `yarn admin destroy --confirm <site>`.
...
- Never ask for or accept a Cloudflare token or microfeed password in chat.
  ...
  exposure.
- Discover authorization and accounts first with `yarn admin accounts --json`.
  ...
  account ID to setup and every later operation. Never pick the first account.
- For built-in login, pass only the sign-in email to setup. Relay the one-time
  password-creation link printed after deployment, let the user choose their
  password in the browser, then rerun `yarn admin status`. Never ask the user
...
- Keep repository deployment tooling in `admin-cli/`. It may import
